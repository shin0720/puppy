...
// ✅ 카드 클릭 시 API 상세 페이지로 이동
  el.addEventListener("click", () => {
    const id = pet.ABDM_IDNTFY_NO || pet.ABDM_IDNTFY_ID || ""; // 고유 ID
    if (id) {
      const url = `https://www.gg.go.kr/animalProtect/detail?ABDM_IDNTFY_NO=${id}`;
...
container.addEventListener("touchstart", (e) => {
  isDragging = true;
  startX = e.touches[0].clientX; // 첫 번째 손가락 위치
  movedX = 0;
});
...
function handleSlideMove() {
  ...
  if (movedX > threshold && currentIndex > 0) {
    currentIndex--;
  } else if (movedX < -threshold && currentIndex < container.children.length - VISIBLE) {
    currentIndex++;
  }
  updateTransform();
}
